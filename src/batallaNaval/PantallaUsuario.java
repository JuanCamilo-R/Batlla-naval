...
private ArrayList<Point> posicionesBarcoActual = new ArrayList<Point>();
	...
	public boolean estaDisponible(int x1,int y1,int x2, int y2,int tamano) {
	...
	private boolean verificarMitad(int x1,int y1,int x2, int y2,int tamano) {
	...
	public void ponerBarco(int x1,int y1,int x2, int y2, int tamano, Barcos barco) {
		...
		if(estaDisponible(x1,x2,y1,y2,tamano)) {
			...
			do {
				if(x1 == x2) {
					posicionesBarcoActual.add(new Point(x1,y1 + contador));
					barcosPantalla[x1][y1+contador] = barco;
				}
				if(y1 == y2) {
					posicionesBarcoActual.add(new Point(x1 + contador,y1));
